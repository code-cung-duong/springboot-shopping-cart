									...
									<ul>
										<c:forEach var="item" items="${categories.listResult}">
											<li class="filter-list"><input  class="pixel-radio"
												type="radio" <c:if test="${item.code == products.uri}">checked</c:if> id="${item.code}" name="radioCa"
												value="${item.code}"><label for="${item.code}">${item.name}<span>(${item.totalItem})</span></label>
											</li>
										</c:forEach>
							...
							<form action="#">
								<ul>
									...
									<c:forEach var="item" items="${brands.listBrand}">
										<li class="filter-list"><input class="pixel-radio"
											type="radio" id="${item.id}" name="brand"><label
							...
							<form action="#">
								<ul>
									<c:forEach var="item" items="${brands.listColor}">
										<li class="filter-list"><input class="pixel-radio"
											type="radio" id="color_${item.id}" name="color"><label
								...
								<div class="col-md-6 col-lg-4">
									<div class="card text-center card-product">
										...
										<div class="card-product__img">
											<img class="card-img" src="web/img/product/${item.image}"
												alt="">
											<ul class="card-product__imgOverlay">
												<li><a href='<c:url value='/${item.uri}'/>'>
														<button>
															<i class="ti-search"></i>
														</button>
												</a></li>
