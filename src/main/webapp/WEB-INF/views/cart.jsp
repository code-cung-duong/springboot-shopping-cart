...
<body>
						...
						<table style="width: 100%;">
							...
							<tbody>
								<form id="formData" autocomplete="off">
									<c:forEach var="item" items="${carts.listResult}">
										...
										<tr>
											...
											<td>
												<div class="product_count">
													<input type="hidden" name="id" value="${item.id}" /> <input
														...
														onchange="dangky(${item.id});" type="text" name="qty"
														id="sst_${item.id}" maxlength="12" value="${item.quanlity}"
											...
											<td>
												<h5 id="tinhTien_${item.id}">$${item.quanlity *
													item.price}</h5>
											</td>
											<td><a href="#!" onclick=" return warningBeforeDelete(${item.id})"><i
								...
								<tr class="bottom_button">
									<td><a href="#!" class="button button-login"
											onclick="return updateCart('cart')">Update
											Cart</a></td>
									<td></td>
									<td></td>
									<td>
										<div class="cupon_text d-flex align-items-center">
											<input type="text" placeholder="Coupon Code"> <a href="#!"
												class="primary-btn">Apply</a> <a onclick="return false" class="button"
												href="#!">Coupon?</a>
										</div>
									</td>
								</tr>
								...
								<tr class="out_button_area">
									...
									<td>
										<div class="checkout_btn_inner d-flex align-items-center">
											<a class="gray_btn" href="/shop">Continue Shopping</a> <a
												onclick="return updateCart('checkout')" class="primary-btn ml-2"
												href="#!">Checkout</a>
										</div>
									</td>
								</tr>
	...
	<c:url var="APIurl" value="/api/cartitem" />
		...
		function updateCart(index) {
			...
			for (var i = 0; i < data.length; i++) {
				if (i % 4 == 0) {
					var list = {};
					list['id'] = data[i];
					list['quanlity'] = data[i + 2];
					list['productId'] = data[i + 3];
					lists[d] = list;
					d++;
				}
			}
